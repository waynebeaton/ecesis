...
public class TwoDimensionalBoard extends Board{
	 ...
	 * Evolves board
	 */
	public void evolve(Game aGame)throws LifeGameException{
		...
		for (int i = 0; i < cells.length; i++){
			for (int j = 0; j < cells[0].length; j++){
				...
				if (!(doesAnyApply))
					throw new LifeGameException("No rules apply for cell[" + i + "][" + j + "]");
			}
		}
	}
	/**
	 ...
	 */
	public void evolve(Game aGame, int index)throws LifeGameException{
		if (index == 0) return;
		for (int i = 1; i <= index; i++){
			evolve(aGame);
			aGame.addToHistory(this);
